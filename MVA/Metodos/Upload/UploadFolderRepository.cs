...
namespace NOTAS.Metodos.Upload
{
    public static class UploadFolderRepository
    {
        ...
        /// <returns></returns>
        public static void UploadDeArquivos(string directoryPath, HttpPostedFileBase[] pathOfFiles)
        {
            foreach (HttpPostedFileBase file in pathOfFiles)
            {
                if (file != null)
                {
                    // Get extension from input file
                    var inputExtensionFile = Path.GetExtension(file.FileName);
                    ...
                    ServerSavePath = Path.Combine(directoryPath + "Uploads", file.FileName);

                    //Save file to server folder  
                    file.SaveAs(ServerSavePath);

                    // Trata o arquivo de acordo com a extensão passada 
                    iFiles.RetornaListaDeNotasFiscaisBaixadas(directoryPath, ServerSavePath, "");
                }
            }
        }
